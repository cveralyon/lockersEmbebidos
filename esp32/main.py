...
url = "http://192.168.100.104:5000/locker/1"
data = {
	"ocupado": True,
	"cerrado": False,
	"hora_ocupacion": "2023-06-28T15:00:00",
	"hora_desocupacion": "2023-06-28T18:00:00",
	"rut": "12345678-9",
	"disponible": False,
	"documento_notif": True,
...
# Here we are using 'urequests' to send a PUT request to update the state
def send_state_to_server(url, data, headers):
		response = urequests.put(url, json=data, headers=headers)
		print(response.text)
		response.close()

# Now, every time the state of a sensor, buzzer, or solenoid changes, 
# we use 'send_state_to_server' to send a PUT request with the updated state
def update_locker_state(locker_name, state_name, new_state):
		global lockers
		lockers[locker_name][state_name] = new_state
		locker_data = lockers[locker_name]
		send_state_to_server(url, locker_data, headers)

# Using the above function to update states
for locker_name, locker in lockers.items():
		update_locker_state(locker_name, 'cerrado', locker['cerrado'])
		update_locker_state(locker_name, 'ocupado', locker['ocupado'])
...
def open_locker(locker_name, retirar=False):
		...
		update_locker_state(locker_name, 'cerrado', False)
		if retirar:
...
def close_locker(locker_name, ingresar=False):
		...
		if locker['locker_vacio'].value() or ingresar:
				locker['relay'].value(CERRADO)
				locker['cerrado'] = True
				update_locker_state(locker_name, 'cerrado', True)
				if ingresar:
						locker['disponible'] = False
						update_locker_state(locker_name, 'disponible', False)
		else:
				print("SONANDO!!!")
				time.sleep(5)
				while lockers[locker_name]["locker_vacio"].value() != 1 or lockers[locker_name]["puerta_cerrada"].value() != 0:
						...
						if lockers[locker_name]["locker_vacio"].value() != 1:
								...
								update_locker_state(locker_name, 'documento_notif', True)
						if lockers[locker_name]["puerta_cerrada"].value() != 0:
								lockers[locker_name]["puerta_abierta_notif"] = True
								update_locker_state(locker_name, 'puerta_abierta_notif', True)
				...
				update_locker_state(locker_name, 'rut', None)
				lockers[locker_name]['disponible'] = True
				update_locker_state(locker_name, 'disponible', True)
				lockers[locker_name]["puerta_abierta_notif"] = False
				update_locker_state(locker_name, 'puerta_abierta_notif', False)
				lockers[locker_name]["documento_notif"] = False
				update_locker_state(locker_name, 'documento_notif', False)
...
def check_locker(locker_name):
		...
		update_locker_state(locker_name, 'ocupado', locker["puerta_cerrada"].value())
		print(locker["ocupado"])

def listen_instructions():
		url = "http://192.168.100.104:5000/instructions"
		global lockers
		response = urequests.get(url)
		if response.status_code == 200:  # Si la petición es exitosa
				message = ujson.loads(response.text)

				if "action" in message:
						if message["action"] == "verify":
								verify_rut(message["rut"])
						elif message["action"] == "open":
								if message["retiro"] == "true":  # Si se está retirando un documento
										for locker_name, locker in lockers.items():
												if locker['rut'] == message["rut"]:
														open_locker(locker_name, True)
														time.sleep(2)
								else:  # Si se están colocando documentos
										locker = message["locker"]
										print('ADMIN COLOCANDO DOCUMENTOS')
										open_locker(locker)
										time.sleep(2)
		...
		while True:
				...
				for locker_name, locker in lockers.items():
						...
						if locker["locker_vacio"].value() == 0:
								...
								update_locker_state(locker_name, 'ocupado', False)
						else:
								locker["ocupado"] = True
								update_locker_state(locker_name, 'ocupado', True)
